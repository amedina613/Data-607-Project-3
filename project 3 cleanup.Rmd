...
```{
```{r}
 soft_vec <-  t(ifelse(t(soft_skills_df)==1,names(soft_skills_df),0))
```
...
```
```{r}
data.frame(primary_key)
```

```{r}
soft_vec2 <- cbind(soft_vec, data.frame(primary_key)) 
soft_vec3 <- soft_vec2[ -c(1) ] %>%
  relocate(primary_key)
